...
export const Address = () => {
  ...
  const SubmitHandler = async (data) => {
    try {
      ...
      data.userId = userId;
      const response = await axios.post(`${API}/api/orders/add-order`,data,{
        headers: {
      ...
      if (response.data.status) {
        ...
        Swal.fire({
          ...
          Navigate(`/order-summary/${response.data.data._id}`);
        });
      } else {
        Swal.fire({
          icon: 'error',
  ...
  return (
    <>
      <Container fluid className="main-form">
        <Row>
          <h2 className="text-center text-white">Order Details</h2>
        </Row>
        <Row>
          <Col>
